...
export const updatePost = async (formData: FormData): Promise<void> => {
  const rawData = { content: exists(formData.get('content')) as string, id: +exists(formData.get('id')) }

  const post = await prisma_updatePost(rawData.id, rawData.content.split('<').join('&lt;').split('>').join('&gt;'))

  await prisma.postMention.deleteMany({ where: { postId: rawData.id } })
  ...
  revalidateTag(`posts?authorId=${post.authorId}`, 'max')
