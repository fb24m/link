...
export const Posts = async ({
  ...
  if (!author) {
    const authors = await prisma.user.findMany({
      where: {
        id: { in: Array.from(new Set(Posts.map((p) => p.authorId ?? 0))) },
      },
    });
    ...
    <div className={styles.posts}>
      <Card className={styles.newDesignReview}>
        ...
        <Popup topCloseButton>
          ...
          <PopupWrapper className={styles.googleFormPopup}>
            <iframe
              src="https://docs.google.com/forms/d/e/1FAIpQLSdC3_z9OoJbUqFYpiN6HwWTfyrhta8boTBJROyE8UoYpVdOPA/viewform?embedded=true"
              width="640"
              height="762"
      ...
      {!pinnedPost && self.id === author?.id && (
        <Card className={styles.pinPost}>
          ...
          <Button appearance="primary" href="/article/190">
            Подробнее
          </Button>
        </Card>
      )}
      ...
      {pinnedPost && (
        <Card className={styles.pinned}>
          <img
            className={styles.blurredAvatar}
      ...
      {(author || whoMentioned) && (
        <Box gap={8} direction="row" alignItems="center">
          <Button
            href={`/user/${author?.username ?? whoMentioned}`}
          ...
          <span className={clsx(styles.new, whoMentioned && styles.primary)}>
            Новое
          </span>
        </Box>
      )}
      ...
      <Card className={styles.end}>
        <h2 className={styles.title}>
          {Posts.length !== 0 ? "Вы дошли до конца" : "Тут пока нет постов"}
        </h2>
        ...
        <Button className={styles.homeButton} href="/" appearance="primary">
          На главную
        </Button>
      </Card>
    </div>
